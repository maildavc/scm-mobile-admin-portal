    ...
    return pages.map((page, index) => {
      if (page === "...") {
        return (
          <span
            key={`ellipsis-${index}`}
            className="w-8 h-8 flex items-center justify-center text-[#2F3140] text-sm font-medium border border-gray-200 bg-white rounded"
          >
            ...
          </span>
        );
      ...
      return (
        <button
          ...
          className={`w-8 h-8 flex items-center justify-center rounded text-sm font-medium border border-gray-200 ${
            isCurrent ? "bg-white text-[#2F3140]" : "bg-white text-[#2F3140] hover:bg-gray-50"
          }`}
        >
          {page}
        </button>
